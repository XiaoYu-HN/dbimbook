...
DECLARE
    ...
    p_cmp_ratio     NUMBER;
    p_comptype_str  VARCHAR2(256);
	p_sample_rows NUMBER := dbms_compression.comp_ratio_allrows;
--	p_sample_rows NUMBER := dbms_compression.comp_ratio_minrows;
    t1              PLS_INTEGER;
...
BEGIN
	FOR i IN 1..comptypes.count LOOP
		...
		dbms_compression.get_compression_ratio(
				...
				comptype_str => p_comptype_str,
				subset_numrows => p_sample_rows,
				objtype => dbms_compression.objtype_table
		);

        dbms_output.put_line('-----------------------------------------------------------------');
        dbms_output.put_line('Compression type                         : ' || p_comptype_str);
